...
function App() {
  ...
  const isAllPagesSelected = selectedPages.length === PAGES_DATA.length && PAGES_DATA.length > 0;
  ...
  return (
    <div className="flex justify-center items-center h-screen">
      <div className="w-92.5 h-81.5 rounded-md border border-[#EEEEEE] shadow-[0px_8px_15px_0px_rgba(20,20,20,0.12),0px_0px_4px_0px_rgba(20,20,20,0.10)]">
        ...
        <div className="border-[#CDCDCD] w-85 border-b mx-auto" />

        <div className="py-2 overflow-y-auto h-45 no-scrollbar">
          {PAGES_DATA.map((page) => (
            <PageSelectorItem
              key={page.id}
              title={page.title}
        ...
        <button className="cursor-pointer rounded-sm bg-[#FFCE22] text-[14px] h-10 w-85 mx-auto block mt-6 hover:bg-[#FFD84D] transition-all duration-150">
          Done
        </button>
      </div>
    </div>
